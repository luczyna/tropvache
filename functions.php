...
if ( ! function_exists( 'boilerplate_posted_on' ) ) :
	...
	function boilerplate_posted_on() {
		...
		// j = 1–31
		// d = 01–31
		printf( __( '<span class="%1$s">Posted on</span> <span class="entry-date">%2$s %3$s %4$s</span>', 'boilerplate' ),
			// %1$s = container class
 ...
 */
	function boilerplate_comment_input_placeholders( $fields ) {
		...
		$fields['email'] = str_replace(
			 ...
			 */
			'<input type="email" placeholder="you@website.com"  id="email" name="email"',
			$fields['email']
		);
		$fields['url'] = str_replace(
			'<input id="url" name="url" type="text"',
			// Again: a better 'type' attribute value.
			'<input placeholder="http://website.com/" id="url" name="url" type="url"',
			$fields['url']
		);
		return $fields;
	}
...
function my_custom_post_project() {
	$labels = array(
		...
		'singular name'      => _x('Project', 'post type singular name'),
		...
		'not_found_in_trash' => __('No projects in da trash'),
		'parent_item_colon'  => '',
		'menu_name'          => 'Projects',
	);
	$args = array (
		'labels'             => $labels,
		'description'        => 'Projects to share',
		'public'             => true,
...
function filter_iframetags_on_images($content){
   return preg_replace('/<p>\s*(<iframe .*>)?\s*(.*)\s*(<\/iframe>)?\s*<\/p>/iU', '\1\2\3', $content);
}
// add_filter('the_content', 'filter_iframetags_on_images');
